...
Accetta un valore di parametri booleani. Se il valore è `True`, il client utilizza la latenza di output audio nel calcolo della marca temporale audio.
...
* Alla `AdClientFactory` classe viene aggiunta una nuova funzione per supportare la registrazione di più Rilevatori opportunità:
...
* **Ad Fallback, concatenamento margherita nella logica di selezione degli annunci (Zendesk #3103** For VAST ads (creativi) con la regola di fallback abilitata, TVSDK tratta un annuncio con un tipo MIME non valido come annuncio vuoto e tenta di utilizzare gli annunci di fallback al suo posto. Potete configurare alcuni aspetti del comportamento di fallback.

Per ulteriori informazioni, vedi [Fallback annunci per annunci](../programming/tvsdk-3x-android-prog/android-3x-advertising/ad-insertion/ad-fallback/android-3x-ad-fallback.md)VAST e VMAP.
...
* Biglietto #34493 - Ritardo audio Bluetooth
   ...
   * Aggiunto `alwaysUseAudioOutputLatency` nella classe MediaPlayer che, se impostata, si traduce nell&#39;utilizzo della latenza di output audio nel calcolo delle marche temporali audio.
...
* Zendesk #22675 La durata delle risorse non viene aggiornata per i flussi Live/Linear. Questo problema è stato risolto fornendo una nuova API, assetDuration, in PTVideoAnalyticsTrackingMetadata che fornisce la durata delle risorse per i flussi Live e Lineari.

* Zendesk #25853 Perdita di memoria in TVSDK quando si cambiano i canaliProblema di perdita di un buffer di lettura dei file quando MediaPlayer viene reimpostato o rilasciato durante il download di un file.

**Versione 1.4.29 (1653)**

* Zendesk #21200 - Il lettore non si riprende dallo stato sospeso quando l&#39;app era in background. Quando il lettore è stato sospeso dopo l&#39;accensione dello streaming, la risoluzione consente al lettore di eseguire l&#39;interruttore dello streaming quando il lettore viene ripristinato dallo stato di sospensione, invece di tornare alla posizione precedente.

* Zendesk #23412 - Il lettore è bloccato con una casella nera se si fa clic su un Annuncio negli ultimi 3 anni dell&#39;interruzione dell&#39;annuncio. Questo problema è lo stesso di Zendesk #21200.

* Zendesk #23616 - Skipped ad break cerca troppo in futuroA seconda del tipo di inserimento degli annunci (inserire/sostituire), TVSDK determina se la durata dell&#39;annuncio viene utilizzata nel calcolo per determinare il punto finale dell&#39;interruzione.

* Zendesk #25078 - TVSDK Perdita di memoria DRM su Android TV STBTla perdita di memoria per l&#39;oggetto adattatore DRM è stata individuata e fissata.
...
* Zendesk #23618 - Eventi di interruzione pubblicitaria attivati prima che la politica dell&#39;annuncio venisse consultataQuesto problema è stato risolto non attivando gli eventi AD_BREAK_START e AD_START quando l&#39;annuncio veniva ignorato a causa del mancato pagamento di un premio. L&#39;evento AD_BREAK_SKIPPED viene inviato.

**Versione 1.4.27 (1631)**

* Zendesk #23174 - Problema di prestazioni durante il ridimensionamento del videoQuesto problema è stato risolto dimostrando una nuova API, MediaPlayerView.setSurfaceFixedSize, che consente a TVSDK di accedere a SurfaceHolder.setFixedSize() da MediaPlayerView.

* Zendesk #24450 - TVSDK effettua richieste pubblicitarie duplicate. Questo problema si verificava quando il tempo trascorso era stato convertito in esteso e non doppio, e questo problema era stato risolto.
...
* Zendesk #22313 Il bitrate per gli STB con chipset Amilogic non va oltre 1,2 MTquesto problema è stato risolto precaricando le funzionalità dei codec multimediali e disabilitando l&#39;interruttore senza soluzione di continuità per i dispositivi chipset Amilogic.

* Zendesk #19520 Esempio di risorsa AES HLS non riprodotta nei lettori TVSDKQuesto problema è stato risolto gestendo più descrittori PMT per flussi HLS crittografati AES di esempio.

**Versione 1.4.23 (1602)**

* Zendesk #18852 - Aggiornare la logica di selezione creativa in base alle regole CRSQuesto problema è stato risolto aggiungendo un file di configurazione JSON per specificare la priorità di selezione creativa.

* Zendesk #20861 - Android NTquesta release fornisce supporto per Android N rimuovendo la capacità di utilizzare direttamente le librerie native della piattaforma Android che non sono più accessibili alle applicazioni che vengono eseguite su Android N.

* Zendesk #21018 - Android N crashStessa risoluzione di ZD# 20861.

* Zendesk #21266 - VideoEngineAdapter si blocca in caso di errore Invalid_KeyL&#39;errore Invalid_Key non include una descrizione da AVE, pertanto l&#39;analisi del testo è risultata in NPE. Il problema è stato risolto aggiungendo un controllo null per la descrizione durante onError prima di analizzare la descrizione.

* Zendesk #22286 - La libreria nativa continua ad allocare la chiave di lettura della memoria/frammento causando l&#39;arresto anomaloQuesto arresto anomalo che si verificava su Android quando si tentava di caricare un manifesto con più chiavi contemporaneamente è stato risolto.

**Versione 1.4.22 (1581)**

* Zendesk #17236 - Tempo inaffidabile per la testina di riproduzione per i video HLS con DRMTè stato corretto il salto temporale con i flussi LBA, dove l&#39;ora di inizio del segmento audio non corrisponde all&#39;ora di inizio del segmento video.

* Zendesk #17680 - La riproduzione video si sta bloccando nella casella di selezione AndredoIl decodificatore video di questo dispositivo a volte restituisce un salto significativo nel tempo di uscita quando si depone il fotogramma video dal buffer di uscita, e questa marca temporale di output rimane alta. Questo problema è stato risolto restituendo un errore di profilo *video non supportato* che non obbliga il lettore a ripetere lo stesso profilo o a scegliere un profilo diverso.

* Zendesk #19074 - Fermo video durante la riproduzione di un trucco FFWD e REWQuesto problema è stato risolto aggiungendo un nuovo avviso TRICKPLAY_ENDED_DUE_TO_ERROR per informare l&#39;applicazione che trickplay è uscito e che il video è stato messo in pausa a causa di un errore irreversibile.

* Zendesk #19574 - TVSDK non restituisce i dati di risposta M3U8 per il contenuto DRM o non DRM. Questo problema è stato risolto nei seguenti modi:
...
* Zendesk #17781 - La schermata ADB non funziona più. Questo problema è stato risolto aggiungendo l&#39;API DefaultMediaPlayer.create(Context context, boolean secureSurface), che consente l&#39;acquisizione dello schermo.
...
* Zendesk #19074 - Fermo video durante la riproduzione di FWD e REWSono stati risolti i seguenti problemi che si verificavano quando il truccoPlay poteva bloccarsi nella riproduzione:
...
* Zendesk #19574 - Il TVSDK non restituisce i dati di risposta M3U8 per il contenuto DRM o non DRMNel caricamento iniziale del file manifesto in PTMediaPlayerItem.PrepareToPlay, se il caricamento non riesce, il TVSDK non segnala il corpo della risposta di errore all&#39;applicazione.
Questo problema è stato risolto consentendo al TVSDK di segnalare la risposta di errore come un errore all&#39;applicazione.

* Zendesk #19701 - Congelamento della riproduzione con SAP/DiscontinuitàIl lettore si blocca quando l&#39;audio e il video non sono allineati alla discontinuità è stato risolto.

* È stato corretto il bug #PTPLAY-11162 - Aggiornamento della libreria OpenSSL alla versione 1.0.2f.

**Versione 1.4.20 (1546)**

* Zendesk #17384 - Richiesta di funzioni: Il supporto per metadati ID3 per la riproduzione AACosupporto per i tag ID3 nei supporti AAC è stato fornito in TVSDK per Android a partire dalla versione 1.4.20.
...
* Zendesk #18229 - SetCCVisiblity(VISIBLE) viene ignorato dopo la chiamata a MediaPlayer.reset(). Questo problema è stato risolto aggiungendo setCCVisibility(Visibility.INVISIBLE); alla funzione reset() nella classe MediaPlayer.

* Zendesk #18328 - È stato risolto un problema di frame su dispositivi Amazon Fire TV di seconda generazione per i contenuti con 60FPST. Questo problema è stato risolto applicando il FPS codificato per il processo decisionale in fase di sospensione e con una logica di previsione FPS meglio codificata.

**Versione 1.4.17 (1472)**

* Zendesk #2231 - Errore restituito dal recupero del manifesto non disponibile in MediaPlayerNotificationQuesto problema è stato risolto includendo il corpo della risposta del manifesto in caso di errore parse.

* Zendesk #17703 - VideoEngineView non impedisce gli screenshot durante la riproduzione videoIl metodo setSecure è disponibile dall&#39;API 17, ma poiché l&#39;API 17 copre 4.2, 4.2.1 e 4.2.2, non è noto quale genererà un&#39;eccezione o se è specifico del dispositivo. Questo problema è stato risolto racchiudendo VideoEngineView.setSecure nella clausola try catch.

* Zendesk #17919 - La ricerca del contenuto causa un errore heartbeatUn errore di posizione dei dati di input non valido si verificava a seguito della chiamata heartbeat generata quando la ricerca veniva avviata dopo il pre-roll. Questo problema è stato risolto.
...
* Zendesk #3875 - Tab S Arresti anomali durante la riproduzioneRipristino della dipendenza di OKHTTP sull’Auditude per CRS perché TVSDK utilizza ora direttamente httpurlconnection anziché curl. Il problema è stato risolto eliminando le eccezioni prima di effettuare qualsiasi altra chiamata JNI.

* Zendesk #4487 - Tracciamento del canale lineare dei contenutiIl problema è stato risolto consentendo la riinizializzazione del tracciatore di heartbeat video durante una sessione di riproduzione di streaming lineare.
...
* Zendesk #18074 - Sottotitoli arabi non funzionanti su Nexus con Android 6.0Il problema è stato risolto fornendo il supporto della mappa dei font CTS per Android.
...
* Zendesk #17851 - Android TV - Black Frame durante l&#39;ABRTla correzione era per specificare KEY_MAX_WIDTH e KEY_MAX_HEIGHT per abilitare la riproduzione adattiva.
...
* Zendesk #3502 - Supporto del failover basato su client HLS durante un&#39;interruzione dell&#39;annuncioConsenti failover al manifesto principale durante l&#39;aggiornamento dell&#39;errore del profilo attivo durante il periodo di interruzione dell&#39;annuncio.

* Zendesk #3875 - Tab S Arresti anomali durante la riproduzionePer risolvere il conflitto tra HttpUrlConnection e cURLm, utilizzare una libreria di terze parti.

* Zendesk #4450: impostazione dei metadati personalizzati per un singolo posizionamento in un risolutore di contenutiAggiunta di un setter alle impostazioni Opportunità.
...
* Zendesk #1218 - 106000.33 Errore con locale ... Se il caricamento del manifesto in FragmentalHTTPStreamer::ThreadParseManifest() non riesce, verificate che il dominio URL sia localhost e, in tal caso, modificate il dominio in 127.0.0.1 e richiamate ThreadParseManifest.
...
* PTPLAY-2736 - Una didascalia WebVTT visualizzata in precedenza viene visualizzata sullo schermo al termine della riproduzione di un flusso con una didascalia WebVTT
...
* Zendesk #2075 - Blocco in VideoEngine.nativeReleaseGPUResource su dispositivi 4.0.xNota: Questa build è ***obbligatorio*** per il supporto di Android 5.0 (Lollipop)
...
* Alcuni dispositivi presentano problemi noti di riproduzione. Consultate Problemi [noti dei dispositivi in 1.4](https://helpx.adobe.com/primetime/release-notes/tvsdk-1-4-android.html#Knownissuesin14).
...
* Bug #3763370 - I flussi live con annunci pubblicitari mostreranno occasionalmente due indicatori di annunci vicini quando ce ne dovrebbe essere solo uno. Questi indicatori di annunci rappresentano lo stesso annuncio pubblicitario, e solo uno verrà riprodotto
* Bug #3763373 - Il marcatore annuncio potrebbe scomparire brevemente quando si cerca oltre un annuncio nei flussi VOD. L&#39;indicatore pubblicitario viene ripristinato e non vi sono altri effetti negativi sulla timeline
* Alcuni dispositivi presentano problemi noti di riproduzione. Per ulteriori informazioni, vedi Problemi [noti dei dispositivi in 1.4](https://helpx.adobe.com/primetime/release-notes/tvsdk-1-4-android.html#Knownissuesin14).
* Implementazione di riferimento - La riproduzione dei mattoni non è implementata nell&#39;applicazione di esempio.
