...
Nei passaggi seguenti viene descritto il flusso di lavoro per il precaricamento della licenza per un file multimediale protetto tramite un `DRMContentData` oggetto.

1. Ottenete i metadati DRM binari per il contenuto del pacchetto.

   Se utilizzate Primetime DRM Java Reference Implementation Packager, questo file di metadati viene generato automaticamente con un&#39; [!DNL .metadata] estensione. Ad esempio, potete scaricare i metadati utilizzando la `URLLoader` classe. Se si utilizza contenuto HLS o HDS, i metadati vengono citati nel file manifesto del contenuto ( [!DNL .m3u8] o [!DNL .f4m]) o inclusi *nel* file manifesto come stringa codificata Base64 (che deve essere decodificata Base64 prima del consumo).
1. Creare un `DRMContentData` oggetto, passando i metadati alla funzione di costruzione:

   ```
   var drmData:DRMContentData = new DRMContentData( metadata );
   ```

1. Gli altri passaggi sono identici al flusso di lavoro descritto in *Dettagli* processo di protezione dei contenuti.
