...
| g | Tipo di dispositivo client | Quando le regole di targeting dipendono dal tipo di dispositivo | Vedere l&#39;elenco in [Tipi di client](https://adobeprimetime.zendesk.com) (richiede l&#39;accesso a Zendesk) |
...
| pabimode | [Abilita ](../../msapi-topics/ms-insert-ads/partial-ad-break-insetion.md) inserimento di interruzioni pubblicitarie parziali Supporto Abilita se true o Avvia Disattiva se false | No (il valore predefinito è disabilitato) | start , true o false |
...
| ptcdn | Elenco di uno o più CDN per l’hosting di risorse transcodificate. Vedere [Supporto per più CDN](../../creative-repackaging-service/multi-cdn-supportt.md). | No (predefinito=Akamai) | Esempio: Akamai, Level3, Limelight, Comcast |
...
| ptfailover | Segnala al server manifesto di identificare i flussi primari e di failover specificati nella playlist principale e di trattarli come set disgiunti. Questo semplifica il failover e impedisce gli errori di temporizzazione. (solo per i dispositivi Apple HLS). Vedere [Facilitazione della commutazione del lettore HLS](../../msapi-topics/ms-insert-ads/hls-switching-to-failover.md). | No | true |
| ptmulticall | Se impostato su true , più chiamate ad Auditude per FER effettuate; uno per ogni annuncio.  Se assente o impostato su false, viene effettuata una chiamata ad auditude per FER. | No | Booleano   Nota:  I seguenti requisiti: <ul><li>il parametro ptcueformat deve essere impostato su nbc</li><li>il parametro pttimeline viene ignorato.</li></ul> |
